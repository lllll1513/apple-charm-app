        ...
        <div className="relative mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <div className="flex -space-x-2 mt-1">
              {p.memberIds.map((id) => (
                <div key={id} className={`h-8 w-8 rounded-full bg-gradient-to-br ${getAvatarColor(id)} ring-2 ring-card/80 flex items-center justify-center text-sm font-medium text-white`}>{getMember(id).name.charAt(0)}</div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className={`h-10 w-10 rounded-full bg-gradient-to-br ${getAvatarColor(p.ownerId)} flex items-center justify-center text-base font-medium text-white`}>{getMember(p.ownerId).name.charAt(0)}</div>
              <div>
                      ...
                      return (
                        <button key={t.id} onClick={() => navigate(`/tasks/${t.id}`)}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className={`h-5 w-5 rounded-full bg-gradient-to-br ${getAvatarColor(t.assigneeId)} flex items-center justify-center text-[10px] font-medium text-white`}>{m.name.charAt(0)}</div>
                          </div>
                        </button>
                      );
                    })}
            ...
            {projectTasks.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => navigate(`/tasks/${t.id}`)} className="w-full grid grid-cols-12 items-center gap-3 px-5 py-3 hover:bg-secondary/40 transition-colors text-left">
                  ...
                  <div className={`col-span-1 h-6 w-6 rounded-full bg-gradient-to-br ${getAvatarColor(t.assigneeId)} flex items-center justify-center text-xs font-medium text-white justify-self-center`}>{m.name.charAt(0)}</div>
            ...
            {p.memberIds.map((id) => {
              ...
              return (
                <Card key={id} className="glass rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:shadow-elevated transition" onClick={() => navigate(`/team/${id}`)}>
                  <div className={`h-12 w-12 rounded-full bg-gradient-to-br ${getAvatarColor(id)} flex items-center justify-center text-lg font-medium text-white`}>{m.name.charAt(0)}</div>
